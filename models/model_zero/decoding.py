...
try:
    ...
    ABS_WEIGHTS = "embeddings/abs-weights.pt"
    EXT_WEIGHTS = "embeddings/ext-weights.pt"
except KeyError:
    print('did you set up environment variables correctly?')
...
def load_best_ckpt(model_dir, reverse=False):
    ...
    ckpt = torch.load(join(model_dir, 'ckpt/{}'.format(ckpts[0])))['state_dict']  # ALES: use parameter map_location to work on cpu !!!

    """
    ALES: You can swap embeddings here 
    """

    import torch.nn.functional as f
    if model_dir.split('/')[-1] == 'abstractor':
        # ckpt['_embedding.weight'] = f.normalize(torch.load(ABS_WEIGHTS))
        ckpt['_embedding.weight'] = torch.load(ABS_WEIGHTS)
        print('ABS weights are loaded')
    else:
        # ckpt['_sent_enc._embedding.weight'] = f.normalize(torch.load(EXT_WEIGHTS))
        ckpt['_sent_enc._embedding.weight'] = torch.load(EXT_WEIGHTS)
        print('EXT weights are loaded')
    return ckpt

"""
DICTIONARY LOAD
abs_dir = '/home/ales/Documents/magisterij/databases/cnndm/fast-abs-sum/pretrained/acl/abstractor'
...
class Abstractor(object):
    ...
    def _prepro(self, raw_article_sents):  # ALES: this function will extend dictionary: for copy purposes
